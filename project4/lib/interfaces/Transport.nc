...
interface Transport{
    ...
    */
   command error_t bind(socket_fd_t fd, socket_addr_t *addr);
    ...
    *    from the pass buffer. This may be shorter then bufflen
    */
   command uint16_t read(socket_t fd, uint8_t *buff, uint16_t bufflen, uint16_t src);
    ...
    *    a closure with the fd passed, else return FAIL.
    */
   command error_t release(socket_fd_t fd);
    ...
    *   to listen else FAIL.
    */
   command error_t listen(socket_fd_t fd);

   command void setTestServer();

   command void setTestClient();

   command void setChatServer();

   command void setChatClient(uint8_t port, uint8_t* username, uint8_t *payload);

   command void msg(uint8_t *payload);

   command void whisper(uint8_t *payload);

   command void printUsers(uint8_t *payload);
