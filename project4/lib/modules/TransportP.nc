...
implementation{
	...
	void connectFinish(socket_t fd) {
		...
		if(chat) {
			...
			while(i < TCP_PACKET_MAX_PAYLOAD_SIZE) {
				myTCPPack->payload[i] = mySocket.sendBuff[i];
  ...
  command uint16_t Transport.write(socket_t fd, uint8_t *buff, uint16_t bufflen){
	...
	myTCPPack->destPort = fd.dest.port;
	myTCPPack->window = 5;
	myTCPPack->seq = sequenceNum++;;
	if(chat) {
		while(buff[i-1] != '\n') {
		myTCPPack->payload[j] = buff[i];
		i++;
		j++;
		//dbg(TRANSPORT_CHANNEL, "HERE%c\n", mySocket.sendBuff[i]);
		if(j == TCP_PACKET_MAX_PAYLOAD_SIZE || buff[i] == '\n') {
			myTCPPack->payload[j] = 0;
			dbg(TRANSPORT_CHANNEL, "HERE %s\n", myTCPPack->payload);
			call Transport.makePack(&myMsg, TOS_NODE_ID, fd.dest.addr, 20, PROTOCOL_TCP, sequenceNum++, myTCPPack, PACKET_MAX_PAYLOAD_SIZE);
			call SendPacketQueue.pushback(myMsg);
  ...
  command uint16_t Transport.read(socket_t fd, uint8_t *buff, uint16_t bufflen, uint16_t src){
  	uint16_t i = 0;
	uint16_t j = 0;
	uint16_t k = 0;
	...
	if(cmd[0] == 'm') {
		...
		while(i < userIndex) {
			...
			while(k < TCP_PACKET_MAX_PAYLOAD_SIZE) {
				myTCPPack->payload[k] = fd.sendBuff[k];
				//dbg(TRANSPORT_CHANNEL, "%c\n", myTCPPack->payload[i]);
				if(fd.sendBuff[k] == '\n') {
					break;
				}
			...
			k = 0;
			dbg(TRANSPORT_CHANNEL, "%s\n", myTCPPack->payload);
			call Transport.makePack(&myMsg, TOS_NODE_ID, 2, 20, PROTOCOL_TCP, sequenceNum++, myTCPPack, PACKET_MAX_PAYLOAD_SIZE);
		...
		dbg(TRANSPORT_CHANNEL, "NEW USER CONNECTED: %s %d\n", user.username, userIndex);
		...
		k = 0;
		dbg(TRANSPORT_CHANNEL, "%s\n", myTCPPack->payload);
		call Transport.makePack(&myMsg, TOS_NODE_ID, user.addr, 20, PROTOCOL_TCP, sequenceNum++, myTCPPack, PACKET_MAX_PAYLOAD_SIZE);
  ...
  command error_t Transport.receive(pack* msg){
		...
		seq = myMsg->seq;
		lastAck = myMsg->lastACK;
		largestAcceptable = myMsg->window;
		flag = myMsg->flag;
		...
		if(flag == SYN_FLAG || flag == SYN_ACK_FLAG || flag == ACK_FLAG){
			...
			if(flag == DATA_FLAG) {
				...
				myTCPPack = (tcp_pack*)(myNewMsg.payload);
				//dbg(TRANSPORT_CHANNEL, "RECEIVED: %s\n",myMsg->payload);
				...
				//if(seq <= mySocket.largestAcceptable && seq > mySocket.lastRcvd) {
					if(chat) {
						...
						for(i = 0; i <= TCP_PACKET_MAX_PAYLOAD_SIZE; i++) {
							mySocket.rcvdBuff[mySocket.lastRcvd + 1] = myMsg->payload[i];
							mySocket.lastRcvd = i;
							//dbg(TRANSPORT_CHANNEL, "%d %d\n", i, myMsg->payload[i]);
							if(myMsg->payload[i] == '\n') {
								dbg(TRANSPORT_CHANNEL, "HERE\n");
								mySocket.lastRcvd = 0;
								call Transport.read(mySocket, mySocket.rcvdBuff, SOCKET_BUFFER_SIZE, msg->src);
								break;
							}else if(myMsg->payload[i] == 0) {
								dbg(TRANSPORT_CHANNEL, "HERE\n");
					...
					}else if(myMsg->payload[0] != mySocket.rcvdBuff[seq]) {
						dbg(TRANSPORT_CHANNEL, "RECEIVED DATA %s\n", myMsg->payload[0]);
						...
						myTCPPack->srcPort = mySocket.src.port;
						myTCPPack->destPort = mySocket.dest.port;
						call Transport.makePack(&myNewMsg, TOS_NODE_ID, msg->src, 20, PROTOCOL_TCP, 0, myTCPPack, PACKET_MAX_PAYLOAD_SIZE);
						
						call Sender.send(myNewMsg, (call RoutingTable.get(msg->src)));
				...
				if(mySocket.ACKed[seq] == FALSE) {
					dbg(TRANSPORT_CHANNEL, "ACK: %d LAST ACK: %d\n", seq, mySocket.lastAck);
	...
	if(myTCPPack->flag == DATA_FLAG){
		...
		while(myTCPPack->seq < mySocket.largestAcceptable && i < (call SendPacketQueue.size())) {
			if(!(call SendPacketQueue.isEmpty())) {
				...
				mySocket = getSocket(myTCPPack->srcPort, myTCPPack->destPort);
				mySocket = activeSockets[1];
				myTCPPack->lastACK = mySocket.lastAck;
				if(mySocket.lastAck == mySocket.transfer) {
					return;
				}else if(myTCPPack->seq > mySocket.lastSent && mySocket.lastSent != mySocket.transfer) {
					dbg(TRANSPORT_CHANNEL, "SENDING...%s\n",  myTCPPack->payload);
					
					mySocket.lastSent = myTCPPack->seq;
				}else if(mySocket.ACKed[myTCPPack->seq] == FALSE) {
					dbg(TRANSPORT_CHANNEL, "TIMEOUT...RETRANSMITTING %d\n", myTCPPack->seq);
   ...
   event void ServerReadTimer.fired(){
	...
	if(!(call RcvdPacketQueue.isEmpty())) {
		...
		mySocket.lastRead = mySocket.lastRead + (call Transport.read(mySocket, mySocket.rcvdBuff, SOCKET_BUFFER_SIZE, myMsg.src));
		activeSockets[mySocket.fd] = mySocket;
	}
   ...
   command void Transport.setChatServer() {
	if(!chatServer) {
		socket_t mySocket;
		socket_addr_t myAddr;
		uint8_t i = 0;
		...
		dbg(TRANSPORT_CHANNEL, "SET UP SERVER AT %d ON PORT %d\n", TOS_NODE_ID, myAddr.port);
	}
}

   command void Transport.setChatClient(uint8_t port, uint8_t* username, uint8_t *payload){
	uint8_t i = 0;
	uint8_t j = 6;
	socket_t mySocket;
	socket_addr_t myAddr;
	...
	activeUsers[1] = user;
	activeSockets[fd] = mySocket;
	dbg(TRANSPORT_CHANNEL, "CLIENT: %s\n", mySocket.sendBuff);

	call Transport.connect(mySocket);
}
    command void Transport.msg(uint8_t *payload) {
	uint8_t i = 0;
	uint8_t j = 0;
	uint8_t k = 0;
	socket_t mySocket = activeSockets[1];
	pack myMsg;
	tcp_pack* myTCPPack;
	user_t user = activeUsers[1];
	...
	myTCPPack->srcPort = mySocket.src.port;
	myTCPPack->destPort = mySocket.dest.port;
	myTCPPack->seq = mySocket.lastAck + 1;
	while(mySocket.sendBuff[i-1] != '\n') {
		myTCPPack->payload[j] = mySocket.sendBuff[i];
		dbg(TRANSPORT_CHANNEL, "%c\n", myTCPPack->payload[i]);
    ...
    command void Transport.whisper(uint8_t* payload) {
	uint8_t i = 0;
	uint8_t j = 8;
	uint8_t k = 0;
	socket_t mySocket = activeSockets[1];
	...
	mySocket.lastWritten = call Transport.write(mySocket, mySocket.sendBuff, SOCKET_BUFFER_SIZE);
	activeSockets[1] = mySocket;
	mySocket.lastWritten = call Transport.write(mySocket, mySocket.sendBuff, SOCKET_BUFFER_SIZE);
	//call SendTimer.startOneShot(1000);
    ...
    command void Transport.printUsers(uint8_t* payload) {
		uint8_t i = 0;
	uint8_t j = 8;
	uint8_t k = 0;
	socket_t mySocket = activeSockets[1];
